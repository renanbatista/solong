...
/*   Updated: 2023/09/12 22:57:32 by r-afonso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_keypress(void *t_c)
{
	...
	else if (mlx_is_key_down(obj->mlx, MLX_KEY_W && *(obj->moviment + 1)))
		obj->img_pac->instances[0].y += 1;
	else if (mlx_is_key_down(obj->mlx, MLX_KEY_A && *(obj->moviment + 0)))
		obj->img_pac->instances[0].x -= 1;
	else if (mlx_is_key_down(obj->mlx, MLX_KEY_D && *(obj->moviment + 2)))
		obj->img_pac->instances[0].x += 1;
	else if (mlx_is_key_down(obj->mlx, MLX_KEY_S && *(obj->moviment + 3)))
		obj->img_pac->instances[0].y -= 1;
	rewrite_player(obj);
}
